...
const Product = () => {
  ...
  const filteredCategory = categories.filter(
    (cat) => cat?.categoryname?.toLowerCase() === (params.id || "").toLowerCase()
    );
      ...
      <div className="bg-[#fdfdfb] flex flex-col gap-5 py-10">
        ...
        <div className="flex container flex-col gap-3">
          ...
          <p className=" text-lg font-sketch md:w-[70%]">
             {filteredCategory.length > 0 ? filteredCategory[0]?.categorydetails:''}
          </p>
        </div>
          ...
          {paginatedProducts.map((product) => (
            ...
            >
              ...
              <div className="p-4 text-center flex font-sketch flex-col gap-2">
                ...
                <Link to={`/product-details/${product._id}`} className="cursor-pointer bg-[#FFD600] text-black font-semibold px-6 py-2 rounded hover:bg-yellow-400 transition">
                  Buy now
                </Link>
              </div>
            </div>
