...
const AdminUpdateGallery = () => {
  ...
  useEffect(() => {
    const fetchGalleryItem = async () => {
      try {
        const res = await fetch(`http://localhost:3000/getgalleryimagebyid/${params.id}`);
        if (!res.ok) throw new Error("Failed to fetch gallery item");
        const data = await res.json();
        console.log(data);
        setFormData({ name: data.name, image: "" });
        setPreview(`http://localhost:3000${data.image}`); // show existing image
      } catch (err) {
        console.log(err);
  ...
  // Submit update
   const handleSubmit = async (e) => {
     ...
     try {
       const response = await fetch(`http://localhost:3000/updateGalleryImage/${params.id}`, {
         method: "PUT",
         body: payload,
       });
